...
- **Warm audio pool & readiness (added 2026-09-12)** — every bundled one-shot is pre-created at boot as a small reusable element pool per SFX (`SFX_POOL_SIZE = 2`, `preload = 'auto'`) plus one pre-created music element reused across races (reset to the top per race; paused — not discarded — by `stopMusic`). The director exposes `warm()` (single-flight; loads every element, resolves only when all are ready, rejects so the readiness tracker can retry) and `warmSnapshot()` (per-sound `idle → warming → ready | failed` + attempt counts). Playback serves from the pool — round-robin, guarded `currentTime = 0`, `play()` rejections swallowed — so no element is constructed during play. Warming joins `src/asset-readiness.ts` as a non-critical `audio` group: silent infinite retry, never gates GO; `?debug` exposes `__raceItAudio()`.
